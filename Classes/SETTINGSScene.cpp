...
Slider* SETTINGSScene::createMusicSlider(Vec2 pos, const Slider::ccSliderCallback& callback)
{
	...
	slider->setTouchEnabled(pic_MS_NODE_path);
	
	if (slider == nullptr) {
		problemLoading("");
		ridio->setBackgroundMusicVolume(0);
	}
	else {
...
Sprite* SETTINGSScene::createCountdownSprite(Vec2 pos)
{
	int countdown = 10; // 倒计时初始值
	...
	if (sprite == nullptr) {
		...
		// 加入倒计时数字和进度条
		auto progress = ProgressTimer::create(Sprite::create("progressbar.png"));
		progress->setType(::ProgressTimer::Type::BAR);
		progress->setMidpoint(Vec2(0, 0.5));
		progress->setBarChangeRate(Vec2(1, 0));
		progress->setPosition(sprite->getContentSize().width / 2, sprite->getContentSize().height / 2);
		sprite->addChild(progress);

		auto label = Label::createWithTTF(std::to_string(countdown), "fonts/arial.ttf", 48);
		label->setPosition(sprite->getContentSize().width / 2, sprite->getContentSize().height / 2 + 50);
		sprite->addChild(label);

		auto countdownCallback = [&](float dt) {
			countdown--;
			label->setString(std::to_string(countdown));
			float percentage = static_cast<float>(countdown) / 10 * 100;
			progress->setPercentage(percentage);

			if (countdown <= 0) {
				Director::getInstance()->getScheduler()->unschedule("countdownCallback", sprite);
			}
		};

		Director::getInstance()->getScheduler()->schedule(countdownCallback, sprite, 1.0f, false, "countdownCallback");
	}

	return sprite;
}
//Sprite* SETTINGSScene::creatProgressTimer(Vec2 pos)
//{
//	auto progress = ProgressTimer::create(Sprite::create("progressbar.png"));
//	progress->setType(ProgressTimer::Type::BAR);
//	progress->setMidpoint(Vec2(0, 0.5));
//	progress->setBarChangeRate(Vec2(1, 0));
//	progress->setPosition(Director::getInstance()->getVisibleSize().width / 2, Director::getInstance()->getVisibleSize().height / 2);
//	return progress;
//}

//Sprite* SETTINGSScene::creatLable(Vec2)
//{
//	auto label = Label::createWithTTF("10", "fonts/arial.ttf", 48);
