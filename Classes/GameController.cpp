...
void GameController::updateCountDown(float dt)
{
	...
	if (timeLeft == 0)
	{
		...
		else
		{
			...
			connection_metadata::ptr metadata = nullptr;
			if (MAINScene::getGameMode() == 2)
			{
				int webId = 0;
				if (metadata == nullptr || metadata->get_status() != "Open")
					webId = client.connect("http://localhost:9002");
				metadata = client.get_metadata(webId);
				while (metadata->get_status() != "Open")
					;
				client.waitEnemyConnection(*metadata);

				auto enemyHeroes = _chessBoard->getEnemyHeroes();

				// 将我方英雄信息发送到我方客户端，并通过服务器传给敌方客户端
				auto myHeroes = _chessBoard->getMyHeroes();
				for (auto it = myHeroes->begin(); it != myHeroes->end(); it++) {
					std::string str = "from " + std::to_string(webId) + " up " + std::to_string(it->heroIdx) + " " + std::to_string(it->level) + " " + std::to_string(it->silentSprite->getPosition().x) + " " + std::to_string(it->silentSprite->getPosition().y);
					client.send(webId, str);
				}
				client.send(webId, "Select Over");
				Sleep(3000);
				//client.waitEnemySelectOver(*metadata); // 阻塞，直到对方选择结束
				
				// 通过服务器传给我方客户端的信息，生成敌方英雄
				metadata = client.get_metadata(webId);
				std::vector<HeroInfo> info;
				if (metadata) {
					client.dealReceicedMessage(*metadata, info);
					if (info.begin() != info.end()) {
						
						for (auto it = info.begin(); it != info.end(); it++) {
							auto enemyHero = getOneEnemy(it->_id, it->_level, Vec2(it->_x, it->_y));
						...
						for (int i = 1; i <= 3; i++) {
							auto enemyHero = getOneEnemy(i, 1, Vec2(300 + 100 * i, 600));
							enemyHeroes->push_front(enemyHero);
						}
					}
				}
			...
			//client.close(webId, close_code, reason);
 ...
 * CopyRight 2023 by 黄辰宇
 * **************************************************/
HeroMsg GameController::getOneEnemy(const int idxOfHero, const int level, const Vec2 pos)
{
	std::string picPath = ConfigController::getInstance()->getPathByID(PATH_SILENT_SPRITE[idxOfHero]);
	auto eSprite = Sprite::create(picPath);

	HeroMsg enemyHero;
	enemyHero.init(nullptr, eSprite, idxOfHero, heroTagCounter++, level);
...
void GameController::gameStart()
{
	...
	for (int i = 1; i <= 5; i++)
	{
		auto relaxPos = Vec2(300 + 100 * i, 600);
		auto enemyHero = getOneEnemy(i, 1, relaxPos);
		enemyHeroes->push_front(enemyHero);
	}
 ...
 * 时间：2024年1月1日
 * **************************************************
 * CopyRight 2024 by 黄辰宇
 * **************************************************/
void GameController::onlineGameStart()
{
...
int GameController::stopFight()
{
	...
	for (std::list<HeroMsg>::iterator it = enemyHeroes->begin(); it != enemyHeroes->end(); it++)
	{
		it->silentSprite->removeFromParentAndCleanup(true);
	}
	enemyHeroes->clear();
