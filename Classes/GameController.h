...
public:
	...
	void sendHeroesToClient(int webId);

	void receiveHeroesFromClient(int webId);

	void executeAfterConnect();
	...
	// 导入敌方英雄信息
	HeroMsg getOneEnemy(const int idxOfHero, const int level, const Vec2 pos);
...
private:
	...
	//static websocket_endpoint client;
	//static connection_metadata::ptr metadata;
