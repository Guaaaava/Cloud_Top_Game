...
//websocket_endpoint* CTGScene::client = nullptr; // 客户端
bool CTGScene::init()
{
	if (!Scene::init())
		return false;
	...
	// 游戏开始
	if (MAINScene::getGameMode() == 1)
		GameController::getInstance()->gameStart();
	else if (MAINScene::getGameMode() == 2) {
		//client = new websocket_endpoint();
		GameController::getInstance()->onlineGameStart();
	}
...
bool CTGScene::onTouchBegan(Touch* touch, Event* event)
{
	...
	{
		// 获取初始位置
		firstPos = touchingHero.silentSprite->getPosition();
		return true;
	}
...
void CTGScene::onTouchMoved(Touch* touch, Event* event)
{
	...
	auto loc = controller->getSizePos(touchPoint);
...
void CTGScene::onTouchEnded(Touch* touch, Event* event)
{
	...
	else if (loc._type == STAND) {
		Vec2 pos = controller->_standOrigin[loc._y][loc._x]->getPosition();
		touchingHero.silentSprite->setPosition(pos);
		controller->_standOrigin[loc._y][loc._x]->setVisible(false);//所在位置设为不可见

		//auto standHeroes = controller->getChessBoard()->getStandHeroes();
		//if (std::find(standHeroes->begin(), standHeroes->end(), touchingHero) == standHeroes->end())
		//	standHeroes->push_front(touchingHero);
	}
	else {
		touchingHero.silentSprite->setPosition(firstPos);

		//auto standHeroes = controller->getChessBoard()->getStandHeroes();
		//if (std::find(standHeroes->begin(), standHeroes->end(), touchingHero) != standHeroes->end())
		//	standHeroes->remove(touchingHero);
	}
}
